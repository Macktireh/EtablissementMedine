...
def sendEmail(subject: str, context: Dict[str, Any], to: List[str], body: str = None, template_name: str = None, _from: str = settings.EMAIL_HOST_USER) -> None:
    try:
        ext = template_name.split('.')[-1]
        htmlContent = get_template(template_name).render(context)
        email = EmailMessage(subject=subject, body=htmlContent if ext == 'html' else body, from_email=_from, to=to)
        if ext == 'html': email.content_subtype = 'html'
